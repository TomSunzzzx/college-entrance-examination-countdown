...
class WindowsManager:
    ...
    def create_win_change_schedule_schedule_adder(cls) -> None:
        global win_schedule_adder

        win_schedule_adder = tk.Tk()
        win_schedule_adder.title('添加一个时间段')
        win_schedule_adder.geometry(f'350x200+{int((win_settings.winfo_screenwidth() - 350) / 2)}+{int((win_settings.winfo_screenheight() - 200) / 2)}')
        GUIObjects.win_settings_frame_change_schedule_win_schedule_adder_element_set()
        win_schedule_adder.mainloop()
...
class GUIObjects:
    ...
    @classmethod
    def win_settings_frame_setting_element_set(cls) -> None:
    ...
    @classmethod
    def win_settings_frame_change_pwd_element_set(cls) -> None:
        global win_settings, frame_change_pwd, entry_win_change_pwd_new_pwd, entry_win_change_pwd_sure_pwd
        ...
        label_win_change_pwd_new_text = ttk.Label(frame_change_pwd, text='新密码', justify=tk.CENTER)
        label_win_change_pwd_new_text.place(x=120, y=80)
        label_win_change_pwd_sure_text = ttk.Label(frame_change_pwd, text='确认密码', justify=tk.CENTER)
        label_win_change_pwd_sure_text.place(x=120, y=130)
        entry_win_change_pwd_new_pwd = ttk.Entry(frame_change_pwd, show='●')
        entry_win_change_pwd_new_pwd.place(x=120, y=100)
        entry_win_change_pwd_sure_pwd = ttk.Entry(frame_change_pwd, show='●')
        entry_win_change_pwd_sure_pwd.place(x=120, y=150)

    @classmethod
    def win_settings_frame_change_schedule_element_set(cls):
        global win_settings, frame_change_schedule, table_frame_change_schedule_show_now_view_page, btn_frame_change_schedule_schedule_view_changer, is_special, now_view
        ...
        btn_frame_change_schedule_schedule_view_changer = ttk.Button(frame_change_schedule, text='查看日常时间表' if is_special else '查看特殊时间表', command=ButtonEvent.schedule_view_changer)
        btn_frame_change_schedule_schedule_view_changer.place(x=100, y=370)
        btn_frame_change_schedule_schedule_changer = ttk.Button(frame_change_schedule, text='修改选中的时间段', command=lambda: ButtonEvent.schedule_changer('change'))
        btn_frame_change_schedule_schedule_changer.place(x=260, y=50, width=125)
        btn_frame_change_schedule_schedule_adder = ttk.Button(frame_change_schedule, text='添加一个时间段', command=lambda: ButtonEvent.schedule_changer('add'))
        btn_frame_change_schedule_schedule_adder.place(x=260, y=100, width=125)
        btn_frame_change_schedule_schedule_remover = ttk.Button(frame_change_schedule, text='删除选中的时间段', command=lambda: ButtonEvent.schedule_changer('remove'))
        btn_frame_change_schedule_schedule_remover.place(x=260, y=150, width=125)
        btn_frame_change_schedule_schedule_import = ttk.Button(frame_change_schedule, text='批量导入', command=lambda: ButtonEvent.schedule_changer('import'))
        btn_frame_change_schedule_schedule_import.place(x=260, y=200, width=125)
        btn_frame_change_schedule_schedule_all_remover = ttk.Button(frame_change_schedule, text='删除全部', command=lambda: ButtonEvent.schedule_changer('remove_all'))
        btn_frame_change_schedule_schedule_all_remover.place(x=260, y=250, width=125)
        label_frame_change_schedule_text_tip = ttk.Label(frame_change_schedule, text='当前程序可以在以下时间段临时关闭', justify=tk.CENTER)
        label_frame_change_schedule_text_tip.place(x=50, y=25)
        table_frame_change_schedule_show_now_view_page = ttk.Treeview(frame_change_schedule, show='headings', columns=('start_time', 'end_time'))
        table_frame_change_schedule_show_now_view_page.column('start_time', width=100, anchor='center')
        table_frame_change_schedule_show_now_view_page.column('end_time', width=100, anchor='center')
        table_frame_change_schedule_show_now_view_page.heading('start_time', text='开始时间')
        table_frame_change_schedule_show_now_view_page.heading('end_time', text='结束时间')
        for i in range(len_schedule := len(now_view)):
            table_frame_change_schedule_show_now_view_page.insert('', i + 1, values=now_view[i])
        table_frame_change_schedule_show_now_view_page.place(x=50, y=50, height=len_schedule * 25 if 0 < len_schedule * 25 < 300 else 300)
        

    @classmethod
    def win_settings_frame_change_schedule_win_schedule_adder_element_set(cls):
        global win_schedule_adder, entry_win_schedule_adder_start_time_hour, entry_win_schedule_adder_start_time_minute, entry_win_schedule_adder_end_time_hour, entry_win_schedule_adder_end_time_minute

        btn_win_schedule_adder_add = ttk.Button(win_schedule_adder, text='添加', command=ButtonEvent.schedule_add)
        btn_win_schedule_adder_add.place(x=150, y=150, width=50)
        label_win_schedule_adder_first_time = ttk.Label(win_schedule_adder, text='开始时间（时/分，24时制）：', justify=tk.CENTER)
        label_win_schedule_adder_first_time.place(x=5, y=30)
        label_win_schedule_adder_end_time = ttk.Label(win_schedule_adder, text='结束时间（时/分，24时制）：', justify=tk.CENTER)
        label_win_schedule_adder_end_time.place(x=5, y=100)
        label_win_schedule_adder_colon1 = ttk.Label(win_schedule_adder, text=':')
        label_win_schedule_adder_colon1.place(x=230, y=30)
        label_win_schedule_adder_colon2 = ttk.Label(win_schedule_adder, text=':')
        label_win_schedule_adder_colon2.place(x=230, y=100)
        entry_win_schedule_adder_start_time_hour = ttk.Entry(win_schedule_adder)
        entry_win_schedule_adder_start_time_hour.place(x=190, y=30, width=30)
        entry_win_schedule_adder_start_time_minute = ttk.Entry(win_schedule_adder)
        entry_win_schedule_adder_start_time_minute.place(x=250, y=30, width=30)
        entry_win_schedule_adder_end_time_hour = ttk.Entry(win_schedule_adder)
        entry_win_schedule_adder_end_time_hour.place(x=190, y=100, width=30)
        entry_win_schedule_adder_end_time_minute = ttk.Entry(win_schedule_adder)
        entry_win_schedule_adder_end_time_minute.place(x=250, y=100, width=30)
...
class ButtonEvent:
    ...
    def sure_password(cls) -> None:
        global password, win_main, config, entry_win_change_pwd_new_pwd, entry_win_change_pwd_sure_pwd, win_settings

        config = InitialOperation.load_config_data()

        if  entry_win_change_pwd_new_pwd.get() == entry_win_change_pwd_sure_pwd.get():
            messagebox.showinfo('提示', '密码设置成功！')
            win_settings.lift()
            win_settings.focus_force()
            cls.return_to_main_frame(frame_change_pwd)
            password = base64.b64encode(bytes(entry_win_change_pwd_new_pwd.get(), 'utf-8')).decode('utf-8')
    ...
    # 时间表转换查看按钮函数
    @classmethod
    def schedule_view_changer(cls) -> None:
        global now_view, btn_frame_change_schedule_schedule_view_changer, table_frame_change_schedule_show_now_view_page, schedule_view_page1, schedule_view_page2
        ...
        if schedule_view_page1 != schedule_view_page2:
            if now_view == schedule_view_page2:
                ...
                for i in table_frame_change_schedule_show_now_view_page.get_children():
                    table_frame_change_schedule_show_now_view_page.delete(i)
                for i in range(len_schedule := len(schedule_view_page1)):
                    table_frame_change_schedule_show_now_view_page.insert('', i + 1, values=schedule_view_page1[i])
                table_frame_change_schedule_show_now_view_page.place(x=50, y=50, height=len_schedule * 25 if 0 < len_schedule * 25 < 300 else 300)
                ...
                for i in range(len_schedule := len(schedule_view_page2)):
                    table_frame_change_schedule_show_now_view_page.insert('', i + 1, values=schedule_view_page2[i])
                table_frame_change_schedule_show_now_view_page.place(x=50, y=50, height=len_schedule * 25 if 0 < len_schedule * 25 < 300 else 300)
    ...
    # 时间表修改按钮函数
    @classmethod
    def schedule_changer(cls, action: str) -> None:
        global schedule_view_page1, schedule_view_page2, now_view
        selected_item = table_frame_change_schedule_show_now_view_page.selection()
        if action == 'add':
            WindowsManager.create_win_change_schedule_schedule_adder()
        elif action == 'remove':
            if selected_item:
                item_index = table_frame_change_schedule_show_now_view_page.index(selected_item[0])
            else:
                return
        elif action == 'import':
            return
    ...
    # 添加时间段界面的
    @classmethod
    def schedule_add(cls):
        global now_view, entry_win_schedule_adder_start_time_hour, entry_win_schedule_adder_start_time_minute, entry_win_schedule_adder_end_time_hour, entry_win_schedule_adder_end_time_minute, btn_frame_change_schedule_schedule_view_changer, win_schedule_adder, win_settings

        schedule = InitialOperation.set_data('set_config_data')[2]

        try:
            start_hour = int(entry_win_schedule_adder_start_time_hour.get())
            end_hour = int(entry_win_schedule_adder_end_time_hour.get())
            start_minute = int(entry_win_schedule_adder_start_time_minute.get())
            end_minute = int(entry_win_schedule_adder_end_time_minute.get())
        except ValueError:
            messagebox.showerror('错误', '无效字符或空值，请输入整数！')
            win_settings.lift()
            win_schedule_adder.lift()
            win_schedule_adder.focus_force()
            return
        
        if start_hour >= 24 or end_hour >= 24 or start_minute >= 60 or end_minute >= 60:
            messagebox.showerror('错误', '数值超出范围！')
            win_settings.lift()
            win_schedule_adder.lift()
            win_schedule_adder.focus_force()
            return

        input_time_range_start = start_hour * 100 + start_minute
        input_time_range_end = end_hour * 100 + end_minute
        
        if input_time_range_start >= input_time_range_end:
            messagebox.showerror('错误', '开始时间必须在结束时间之前且不得与结束时间相同！')
            win_settings.lift()
            win_schedule_adder.lift()
            win_schedule_adder.focus_force()
            return
        ...
        if now_view:
            if btn_frame_change_schedule_schedule_view_changer.cget('text') == '查看特殊时间表':
                for i in range(len(schedule_normal := schedule['normal'])):
                    time_range1 = int(schedule_normal[i][0].split(':')[0]) * 100 + int(schedule_normal[i][0].split(':')[1])
                    time_range2 = int(schedule_normal[i][1].split(':')[0]) * 100 + int(schedule_normal[i][1].split(':')[1])
                    if max(input_time_range_start, time_range1) <= min(input_time_range_end, time_range2):
                        messagebox.showerror('错误', '所填时间不能与已有时间有交集！')
                        win_settings.lift()
                        win_schedule_adder.lift()
                        win_schedule_adder.focus_force()
                        break
