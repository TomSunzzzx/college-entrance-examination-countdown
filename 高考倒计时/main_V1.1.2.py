...
# import json  # 处理json格式数据（还没开发设置功能，先放着）
import os
import sys  # 用于获取文件路径
...
admit_close_time_weekdays = [[datetime.time(7, 59), datetime.time(8, 40)],
                             ...
                             [datetime.time(10, 8), datetime.time(10, 50)],
...
# 程序路径，注意：如果是用pyhon解释器运行的话，显示的是解释器路径！
# 不妨碍正常操作！
# 编译之后才有效果！
...
# 获取时间、文字函数
def get_resource() -> tuple[list[str], str]:
...
def button_fresh() -> None:
    ...
    # 时间
    text2 = tk.Text(win, height=2, width=3)
    font1 = font.Font(size=35)
    text2.insert(tk.END, tm, 'red')
    tk.Text.tag_configure(text2, 'red', foreground='red')
    text2.configure(bg=win.cget('bg'), font=font1,
...
def author_info():
    ...
    f = font.Font(size=5)
    t.configure(bg=win2.cget('bg'), font=f, state='disabled',
...
def create_win():
    ...
    b5 = tk.Button(win, text='设置', width=5, height=1,
                   command=settings)  # 功能未开发
    ...
    text.configure(bg=win.cget('bg'), font=font1,
                   state='disabled', borderwidth=0, highlightthickness=0)
    ...
    text3.configure(bg=win.cget('bg'), font=font1,
                    state='disabled', borderwidth=0, highlightthickness=0)
    ...
    text5.configure(bg=win.cget('bg'), state='disabled',
                    borderwidth=0, highlightthickness=0)
    ...
    # 路径文本
    text6 = tk.Text(win, height=1.3, width=50)
    text6.insert(tk.END, f'程序路径：{path}', 'red')
    tk.Text.tag_configure(text6, 'red', foreground='red')
    text6.configure(bg=win.cget('bg'), state='disabled',
                    borderwidth=0, highlightthickness=0)

    # 设置文本位置左下角
    text6.pack(side='left', anchor='se')
